...
19. [Role-Based Access Control (RBAC)](#role-based-access-control-rbac)
20. [Notification Routing](#notification-routing)
21. [Supplier Self-Service Portal](#supplier-self-service-portal)
22. [Data Lineage UI](#data-lineage-ui)
23. [XBRL / Inline XBRL Export](#xbrl--inline-xbrl-export)
24. [What-If Simulator (ROI page)](#what-if-simulator-roi-page)
25. [Deployment](#deployment)
26. [CI & Repository Hygiene](#ci--repository-hygiene)
...
Implementation: `utils/rbac.py`. UI: `pages/13_Team.py`. Storage: extends the `User` dataclass in `utils/user_store.py` with two new fields.

### Roles & permissions

Four roles, eleven permissions. The `_MATRIX` constant in `utils/rbac.py` is the single authoritative grant path — adding a permission name to `PERMISSIONS` without listing it in `_MATRIX` deliberately leaves it ungranted.
...
`User` gained `org_id` and `org_name` fields. Older user records on disk lack these — `_user_from_record()` in `utils/user_store.py` filters JSON keys to declared dataclass fields, so future schema additions and pre-migration records both load cleanly.

`auth.signup()` rules:

1. If the caller didn't pass `org_id`, default to `org_<username>` from `rbac.default_org_for()`. Single-user demos work without any explicit team setup.
2. If no other user is already in the resolved org, the new user is auto-promoted to `admin` regardless of the requested role. This guarantees every workspace always has someone who can manage members — without it, a fresh org with all viewers would be permanently locked.

### Gate API

```python
from utils.rbac import require_permission, has_permission

# At the top of an admin-only page
require_permission("manage_users")  # renders denied panel + st.stop() on miss

# Inline branches
if has_permission(current_user(), "manage_xbrl"):
    st.download_button("Download XBRL", ...)
```

`require_permission` returns the current user dict on success so callers don't double-look-up.

### Telemetry & failure modes

- Unknown permission strings return `False`. A typo never silently grants access.
- Unknown roles return `False`. A user record with `role="superuser"` cannot do anything.
- `update_role()` validates against `ROLES` and raises `ValueError` on a typo so the UI sees the failure rather than silently writing a permanently-locked-out account.

`tests/test_rbac.py` pins all the above.

---

## Notification Routing

Module: `utils/notifications.py`. UI: `pages/14_Notifications.py`. Wired into framework refresh, orchestrator pipeline completion, risk predictor threshold, and supplier portal.

### Channels

| Channel | Transport | Auth | Renderer |
| --- | --- | --- | --- |
| `email` | `smtplib` STARTTLS | SMTP env vars (see below) | `_render_email()` produces `(subject, text_body, html_body)` |
| `slack` | `POST` to incoming webhook | URL is the credential | `_render_slack()` produces blocks with optional `actions` button |
| `teams` | `POST` to incoming webhook | URL is the credential | `_render_teams()` produces a `MessageCard` with severity-coloured theme |
| `generic_webhook` | `POST` JSON | URL is the credential | `_render_generic()` produces a flat dict with the full event |

### Event taxonomy (`EVENT_TYPES`)

| Event | Fired from | Severity rule |
| --- | --- | --- |
| `regulatory_update_pending` | `utils/framework_refresh.py:_notify_pending_updates` | `warning` if ≥3 new updates, else `info` |
| `pipeline_run_completed` | `core/orchestrator.py:_notify_pipeline_complete` | `info` |
| `pipeline_run_failed` | same | `warning` |
| `risk_threshold_breached` | `agents/risk_predictor.py:_maybe_notify_risk_breach` | `critical` at ≥85, `warning` 70–84 |
| `supplier_submission_received` | `pages/15_Supplier_Portal.py` | `info` |
| `test` | `pages/14_Notifications.py` Test button | `info` |

### Severity gating

Routes carry a `min_severity` (`info` < `warning` < `critical`) and an optional `event_types` filter. Empty filter means "all events". `_severity_rank()` maps the labels to integers; `Route.matches()` returns `False` when:

- `route.enabled` is `False`, or
- `event.type` is not in `route.event_types` (when the filter is non-empty), or
- `_severity_rank(event.severity) < _severity_rank(route.min_severity)`.

### Dispatch isolation

`notify(event, owner=...)` collects matching routes and walks them through `_dispatch_to(route, event)`. Each transport call is wrapped in its own try/except inside `_dispatch_to`, so:

- A 500 from one Slack workspace never blocks delivery to a sibling Teams channel.
- An exception never bubbles into the pipeline run / framework refresh / risk evaluation / supplier submission that triggered the notification.

The `DispatchResult` dataclass captures the outcome per route (`ok`, `detail`) so the **🧪 Test** button on the notifications page can render success / failure inline.

### Per-org route store

`RouteStore` mirrors `utils.run_store` — HF Dataset preferred, local JSON fallback (`data/notification_routes/`), 60-second TTL cache, structured `diagnostic()` for the sidebar. Routes are stored at `notification_routes/<safe_owner>.json` in the auth dataset.

### Configuration env vars

| Var | Required for | Notes |
| --- | --- | --- |
| `ESG_SMTP_HOST` | Email channel | If unset, email channel reports "No SMTP config" instead of attempting send |
| `ESG_SMTP_PORT` | Email channel | Default `587` |
| `ESG_SMTP_USER` / `ESG_SMTP_PASS` | Email channel | Optional auth credentials |
| `ESG_SMTP_SENDER` | Email channel | `From:` address — required to enable email |
| `ESG_SMTP_TLS` | Email channel | `1` (default) for STARTTLS, `0` for plain |
| `ESG_DEFAULT_ORG` | All channels | Fallback org id when notifications fire from a non-Streamlit context (CLI / cron / background thread) |

`tests/test_notifications.py` pins the matrix above plus the dispatch-isolation invariant.

---

## Supplier Self-Service Portal

Modules: `utils/supplier_tokens.py`. Pages: `pages/15_Supplier_Portal.py` (public, token-only) and `pages/16_Supplier_Inbox.py` (admin / analyst).
...
@dataclass
class SupplierToken:
    token: str          # 32-byte URL-safe (~43 chars) from secrets.token_urlsafe
    supplier_name: str
    org_id: str         # the buying org issuing the link
    period: str         # "Q3 2026", "FY2026", ...
    contact_email: str
...
`SupplierToken.is_valid()` returns `(ok, reason)` and surfaces errors in priority order:

1. **revoked** — "contact your buyer for a new one"
2. **expired** — "this link expired on ..."
3. **already used** — "one submission per link"

The order matters because the most actionable message is the one we want the supplier to see. Malformed expiry strings are treated as no-expiry rather than blocking the supplier on a parse error.

### Submission validation

`validate_submission(rows)` returns `(clean_rows, errors)`. Rules:

- Allowed fields are whitelisted in `ALLOWED_FIELDS` — unknown fields (including injection attempts like `<script>`) are silently dropped.
- `NUMERIC_FIELDS` are coerced via `float()`; non-numeric values reject the row with an error.
- Empty rows (every value None / empty / 0) are rejected.
- Non-dict entries reject with a clear "row N: not a key/value record".

### Single-use flow

1. Admin / analyst on `pages/16_Supplier_Inbox.py` mints a token via `SupplierToken.mint()`. The page prints a copy/paste URL — fully-qualified if `ESG_PUBLIC_URL` is set, path-only otherwise.
2. Supplier opens `/Supplier_Portal?t=<token>`. The page resolves the token via `TokenStore.find_token()` — which walks every known org's token file because the supplier doesn't know who issued the link — and validates it.
3. Supplier fills the typed form, ticks the consent checkbox, submits.
4. Submitted rows go through `validate_submission()`. On success we call `SubmissionStore.add_submission()` and `TokenStore.patch_token(used_at=...)` to make the link single-use. A `supplier_submission_received` notification fires.
5. Admin opens the inbox, reviews the row(s), clicks **Merge into supply_chain**. We append to whatever's currently published on the per-user `dataset_supply_chain` channel and re-publish — the next pipeline run sees the merged data.

### Persistence layout

```
supplier_tokens/<safe_org>.json        # array of SupplierToken records
supplier_submissions/<safe_org>.json   # array of Submission records
```

Same backend resolver (HF Dataset → local JSON) and TTL cache pattern as the other stores.

### Public-page surface

`pages/15_Supplier_Portal.py` deliberately:

- Does **not** call `require_login()` — the token *is* the credential.
- Does **not** call `sidebar_auth_widget()` — suppliers should never see app pages.
- Injects CSS to hide `[data-testid="stSidebarNav"]` so the nav doesn't render even briefly.

`tests/test_supplier_tokens.py` pins token lifecycle, validation rules, and the priority order in `is_valid()`.

---

## Data Lineage UI

Page: `pages/17_Data_Lineage.py`. Permission: `view_lineage`.

### What it renders

1. **Sankey diagram** — four columns of nodes:
   - `src · …` — registered connectors (or a single "(sample data)" placeholder when no connector is registered)
   - `schema · …` — canonical ESG schemas (`emissions`, `esg_metrics`, `supply_chain`, `energy`, `waste`, `diversity`, `financials`)
   - `agent · …` — the nine pipeline agents
   - `out · …` — published `state_manager` channels (e.g. `roi_results`)

   Links: source → schema (weighted by `last_row_count`), schema → agent (per the static `AGENT_SCHEMA_INPUTS` map), agent → agent (per `PIPELINE_DEPS` from the orchestrator), agent → output. A fully populated output channel renders with a strong colour; an empty channel renders faded.

2. **Per-agent table** — what each agent reads, what it publishes, current status from `agent_telemetry.load_all()`, last run timestamp + runtime, and whether the output channel has data right now.

3. **Configured data sources** — connector type, target schema, status, last fetch, row count.

4. **Live channel inventory** — `state_manager.get_all_channels()` rendered as a table for debugging "why doesn't the downstream agent see my data?".

### What it deliberately does NOT do

- Does not run any agents. Pure metadata read — safe to land on while the pipeline is mid-run.
- Does not modify any state. Auditor-friendly — viewers can give it to a regulator without worrying about side effects.
- Does not require Plotly — falls back to the per-agent table when Plotly isn't installed.

---

## XBRL / Inline XBRL Export

Module: `utils/xbrl_export.py`. UI: download buttons added to `pages/5_Report_Generator.py`, gated on `manage_xbrl`.

### Public API

```python
from utils.xbrl_export import (
    extract_facts,        # report dict → list[Fact]
    build_xbrl_instance,  # report dict → XBRL 2.1 XML string
    build_inline_xbrl,    # report dict → iXBRL HTML string
    build_facts_csv,      # report dict → CSV string for cross-checking
)
```

### Fact taxonomy (this iteration — quantitative facts only)

| Concept | Source field in Report Generator output | Unit |
| --- | --- | --- |
| `TotalGreenhouseGasEmissions` | `carbon_highlights.total_emissions` | `tCO2e` |
| `ScopeOneEmissions` | `scope_totals_current["Scope 1"]` | `tCO2e` |
| `ScopeTwoEmissions` | `scope_totals_current["Scope 2"]` | `tCO2e` |
| `ScopeThreeEmissions` | `scope_totals_current["Scope 3"]` | `tCO2e` |
| `OverallRegulatoryCompliancePercent` | `compliance_summary.overall` | `Pure` |
| `Compliance{Framework}Percent` | `compliance_summary.frameworks[fw]` | `Pure` |
| `ESGLinkedCapitalExpenditure` | `roi_summary.total_esg_capex` | ISO-4217 currency |
| `ESGNetFinancialBenefit` | `roi_summary.net_financial_benefit` | ISO-4217 currency |
| `ESGFinancialReturnPercent` | `roi_summary.roi_pct` | `Pure` |
| `ESGInvestmentQualityScore` | `investment_quality.score` | `Pure` |

Concept names are deliberately PascalCase + alphanumeric — `_concept_for_framework()` strips dashes and spaces (`SEC-Climate Rule` → `ComplianceSECClimateRulePercent`) so the output is XML-safe.

Missing or `"N/A"` values are skipped, **not** tagged as zero. XBRL consumers (XBRL US, SEC EDGAR) treat absent and zero very differently.

### Entity identifier

`_entity_identifier()` resolves in priority order:

1. `company.lei` — emits `<xbrli:identifier scheme="http://standards.iso.org/iso/17442">`. This is what the SEC and EFRAG accept.
2. Sanitised company name — emits `scheme="http://esg-pilot.dev/identifier"`. Falls back so the export still validates structurally on the demo data.

### Taxonomy URI

Default: `ESG_NS = "http://esg-pilot.dev/taxonomy/2026"`. For a real filing, replace this constant with the regulator's published taxonomy URI (e.g. `http://www.efrag.org/xbrl/esrs/2026` for CSRD ESRS, or the SEC Climate equivalent). Concept names already align with ESRS / SEC concept names so the swap is mechanical — no consumer-side rewrites required.

### Output formats

- **`build_xbrl_instance()`** — a self-contained XBRL 2.1 instance document. Contexts and units are deduplicated; one `<xbrli:context>` per unique `(period_start, period_end)` pair.
- **`build_inline_xbrl()`** — XHTML wrapping the same facts in `<ix:nonFraction>` elements with `name`, `contextRef`, `unitRef`, `decimals`. Contexts and units live inside `<ix:resources>` in a hidden header div per the iXBRL spec. SEC EDGAR and EFRAG ESEF accept this format.
- **`build_facts_csv()`** — flat CSV (`concept,value,unit,decimals,period_start,period_end`) for cross-checking against the dashboard.

### Validation & limitations

- Output is well-formed XML / XHTML — `xml.etree.ElementTree` parses both successfully (tested in `tests/test_xbrl_export.py`).
- **Schema validation** against a published taxonomy XSD is not performed — the synthetic ESG namespace doesn't have a public XSD. Wire your regulator's XSD in when filing for real.
- **Narrative block-tagging** (CSRD ESRS narrative concepts) is intentionally out of scope for this iteration. Add `<ix:nonNumeric>` wrappers in `build_inline_xbrl()` when needed.
