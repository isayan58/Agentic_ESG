...
from utils.ui import (
    ...
    narrative_card, hypothesis_cards,
)
    ...
    if "error" in results:
        ...
        if _section == '📊 Your Company':
            ...
            section_header("Executive Briefing",
                           "The ROI Agent's written read on this run — the "
                           "story behind the numbers above.")
            narrative_card(
                results.get("narrative") or "",
                source="Written by the ESG ROI Agent from this run's data",
            )

            section_header(
                "Does ESG Actually Work Here?",
                "Four claims people make about ESG, each judged against this "
                "run's own numbers.",
            )
            callout(
                "These are the questions the whole platform exists to answer. "
                "**Supported** means this run's data backs the claim, "
                "**Mixed** means it is partly there, **Not yet** means the "
                "evidence isn't present — which is useful to know honestly.",
                icon="🔬",
            )

            _growth = _num(fin_summary.get("revenue_growth_pct"))
            _savings = _num(fin_roi.get("cost_savings", {}).get("total"))
            _breakeven = results.get("j_curve", {}).get("breakeven_quarter")
            _has_breakeven = bool(_breakeven) and _breakeven != "Not yet reached"

            hypothesis_cards([
                {
                    "code": "H1",
                    "question": "Does ESG help the business grow?",
                    "evidence": f"{_growth:.1f}%",
                    "evidence_label": "revenue growth this year",
                    "verdict": ("supported" if _growth >= 10 else
                                "mixed" if _growth >= 3 else "not_supported"),
                    "note": "Growth at or above 10% is a strong signal; "
                            "3–10% is real but modest.",
                },
                {
                    "code": "H2",
                    "question": "Does ESG cut costs and lift profit?",
                    "evidence": f"INR {_savings:,.0f} Cr",
                    "evidence_label": "cost savings identified",
                    "verdict": ("supported" if _savings > 0 else "not_supported"),
                    "note": "Any positive figure means ESG work is paying for "
                            "part of itself already.",
                },
                {
                    "code": "H5",
                    "question": "Is ESG spend a good investment?",
                    "evidence": f"{iqs_score:.0f}/100",
                    "evidence_label": f"investment quality — grade {iqs_grade}",
                    "verdict": ("supported" if iqs_score >= 70 else
                                "mixed" if iqs_score >= 50 else "not_supported"),
                    "note": "70+ is the level where ESG capex stands up as an "
                            "investment case on its own.",
                },
                {
                    "code": "H6",
                    "question": "Does ESG cost first and pay back later?",
                    "evidence": (str(_breakeven) if _has_breakeven
                                 else "Not yet reached"),
                    "evidence_label": ("is when costs are recovered"
                                       if _has_breakeven
                                       else "— still in the investment phase"),
                    "verdict": ("supported" if _has_breakeven else "pending"),
                    "note": ("Passing breakeven confirms the J-curve played out "
                             "as expected." if _has_breakeven else
                             "Not a failure — most programmes spend before they "
                             "save. Watch this each run."),
                },
            ])
