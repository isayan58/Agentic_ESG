...
# ---------------------------------------------------------------------------
# Vitality layer — motion and highlight on top of the base system
# ---------------------------------------------------------------------------
# The palette does not change here: still PwC orange on white. What changes
# is that the page arrives rather than appears. Cards rise in with a short
# stagger, bars grow to their value instead of being drawn at it, and
# anything interactive answers the cursor.
#
# Everything is deliberately short (200–500ms) and runs once on mount. The
# global prefers-reduced-motion rule in _STATIC_CSS matches `*`, so every
# animation added here is switched off for that preference without needing
# to be listed again.
_VITALITY_CSS = """
<style>
/* ── Motion primitives ───────────────────────────────────────────── */
...
_GLOBAL_CSS = _TOKEN_CSS + _STATIC_CSS + _VITALITY_CSS
...
# ---------------------------------------------------------------------------
# Narrative card + hypothesis verdicts
# ---------------------------------------------------------------------------
_NARRATIVE_CSS = """
<style>
/* Executive briefing — LLM prose given a frame and an attribution ----- */
.esg-brief {
    position: relative;
    padding: var(--space-5) var(--space-6) var(--space-5) var(--space-7);
    border: 1px solid var(--border); border-radius: var(--radius-lg);
    background:
        radial-gradient(520px 200px at 0% 0%, rgba(253, 81, 8, 0.07), transparent 62%),
        var(--surface-raised);
    box-shadow: var(--shadow-sm);
}
.esg-brief::before {
    ...
    font-size: 3.4rem; line-height: 1; color: var(--pwc-orange); opacity: 0.30;
}
.esg-brief-body {
    font-size: var(--text-md); line-height: var(--lh-relaxed);
    color: var(--text-secondary);
}
.esg-brief-body p { margin: 0 0 var(--space-3) 0; }
.esg-brief-body p:last-child { margin-bottom: 0; }
.esg-brief-foot {
    display: flex; align-items: center; gap: var(--space-2);
...
.esg-brief-dot {
    ...
    background: var(--pwc-success); flex-shrink: 0;
}
...
.esg-hyp-card {
    ...
    border: 1px solid var(--border); border-radius: var(--radius-lg);
    background: var(--surface-raised); box-shadow: var(--shadow-sm);
    border-top: 3px solid var(--text-disabled);
    transition: transform var(--dur-base) var(--ease-standard),
                box-shadow var(--dur-base) var(--ease-standard);
}
.esg-hyp-card:hover { transform: translateY(-2px); box-shadow: var(--shadow-md); }
.esg-hyp-top {
...
.esg-hyp-q {
    font-family: var(--font-display); font-weight: 700;
    font-size: var(--text-base); line-height: var(--lh-snug);
    color: var(--text); letter-spacing: -0.01em;
}
.esg-hyp-ev {
...
.esg-hyp-ev strong {
    ...
    font-size: var(--text-lg); color: var(--text); letter-spacing: -0.02em;
    margin-bottom: 1px;
}
.esg-hyp-note {
...
_HYP_VERDICTS = {
    "supported":     ("var(--pwc-success)", "rgba(46, 133, 64, 0.10)",  "rgba(46,133,64,0.32)",  "✓ Supported"),
    "mixed":         ("#9a6b00",            "rgba(255, 182, 0, 0.16)",  "rgba(255,182,0,0.45)",  "~ Mixed"),
    "not_supported": ("var(--pwc-danger)",  "rgba(200, 16, 46, 0.09)",  "rgba(200,16,46,0.30)",  "✗ Not yet"),
    "pending":       ("var(--text-muted)",  "rgba(91, 100, 115, 0.08)", "rgba(91,100,115,0.24)", "• Pending"),
}
...
def narrative_card(
    text: Any,
    *,
    source: str = "Generated by the agent from this run's data",
) -> None:
    """Frame agent-written prose so it reads as a briefing, not loose text.

    ``st.write`` on an LLM narrative gives an unattributed wall of words
    with nothing to signal where it came from or how far it runs. The
    quote frame plus a source line does both.

    Accepts a string *or* a list of paragraphs — agents differ on which
    they emit (the ROI agent returns a list, the carbon agent a string),
    and a briefing card is not the place to be fussy about it.
    """
    _narrative_css()
    ...
    if not chunks:
        chunks = ["No narrative was generated for this run."]
    paragraphs = "".join(f"<p>{html.escape(c)}</p>" for c in chunks)
    body = " ".join(chunks)
    words = len(body.split())
    read_s = max(1, round(words / 200))
    st.markdown(
        f'<div class="esg-brief"><div class="esg-brief-body">{paragraphs}</div>'
        f'<div class="esg-brief-foot"><span class="esg-brief-dot"></span>'
        f'{html.escape(source)} · ~{read_s} min read</div></div>',
        unsafe_allow_html=True,
    )


def _narrative_css() -> None:
    inject_global_css()
    st.markdown(_NARRATIVE_CSS, unsafe_allow_html=True)


def hypothesis_cards(items: Sequence[dict]) -> None:
    """Show each business hypothesis with a verdict, not just a number.

    The table this replaces listed a code ("H1 Growth"), a claim, and a
    reading — but never said whether the reading *supported* the claim,
    which is the only thing anyone opens it to learn.

    Each item: ``code``, ``question``, ``evidence``, ``evidence_label``,
    ``verdict`` (supported/mixed/not_supported/pending) and ``note``.
    """
    _narrative_css()
    cards = []
    for item in items or []:
        verdict = str(item.get("verdict") or "pending")
        color, bg, border, label = _HYP_VERDICTS.get(verdict, _HYP_VERDICTS["pending"])
        code = str(item.get("code") or "")
        code_html = (f'<span class="esg-hyp-code">{html.escape(code)}</span>'
                     if code else "<span></span>")
        note = str(item.get("note") or "")
        note_html = (f'<div class="esg-hyp-note">{html.escape(note)}</div>'
                     if note else "")
        cards.append(
            f'<div class="esg-hyp-card" style="border-top-color:{color};" '
            f'role="group" aria-label="{html.escape(str(item.get("question","")))} '
            f'— {html.escape(label)}">'
            f'<div class="esg-hyp-top">{code_html}'
            f'<span class="esg-hyp-verdict" style="color:{color};background:{bg};'
            f'border-color:{border};">{html.escape(label)}</span></div>'
            f'<div class="esg-hyp-q">{html.escape(str(item.get("question", "")))}</div>'
            f'<div class="esg-hyp-ev"><strong>'
            f'{html.escape(str(item.get("evidence", "—")))}</strong>'
            f'{html.escape(str(item.get("evidence_label", "")))}</div>'
            f'{note_html}</div>'
        )
    st.markdown(f'<div class="esg-hyp">{"".join(cards)}</div>',
                unsafe_allow_html=True)
